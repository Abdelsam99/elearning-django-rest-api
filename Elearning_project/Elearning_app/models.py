...
class Profile(models.Model):
    ...
    type_profil = models.CharField(
        max_length=20, choices=TYPE_USER, default=student)
...
class Course(models.Model):
    ...
    slug = models.SlugField(null=True, blank=True)
    description = models.TextField(null=True, blank=True)
    filePresentation = models.ImageField(
        upload_to="courseImage/", null=True, blank=True)
    student = models.ManyToManyField('Profile', blank=True)
    autor = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name='products')
    categoryId = models.ForeignKey(
        Categorie, on_delete=models.CASCADE, related_name='categories')
...
class Lesson(models.Model):
    name = models.CharField(max_length=255)
    slug = models.SlugField(null=True, blank=True)
    video = models.FileField(upload_to='lessonvideos/', validators=[FileExtensionValidator(
        allowed_extensions=['mp4', 'avi', 'mov', 'mkv'])], null=True, blank=True)
...
class AnswerNotice(models.Model):
    ...
    notice = models.ForeignKey(
        Notice, on_delete=models.CASCADE, null=True, blank=True)
    course = models.ForeignKey(Course, null=True, on_delete=models.CASCADE)
    autor = models.ForeignKey(User, on_delete=models.CASCADE)
    date = models.DateTimeField(auto_now_add=True)
